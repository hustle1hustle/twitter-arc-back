    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          // Проверяем на "Not Found" и другие ошибки
          if (result.message === "Not Found" || result.error) {
            resolve({ error: result.message || result.error });
          } else {
            resolve(result);
          }
        } catch (e) {
...
function computeReputation(user, smartFollowers, meta) {
  ...
  // Правильная обработка smart followers
  let smartTop = [];
  let smartCount = 0;
  
  if (smartFollowers && !smartFollowers.error && smartFollowers.smart_followers) {
    smartTop = smartFollowers.smart_followers.slice(0, 5).map(s => `@${s.screen_name}`);
    smartCount = smartTop.length;
  }
  ...
  const repScore = Math.round(
    ...
    0.10 * ((meta && !meta.error ? meta.avg_smart_score : 0) / 10)
  );
  ...
  return {
    ...
    followers: followers,
    smartTop,
    smartMedianFollowers: meta && !meta.error ? meta.median_followers || 0 : 0,
    smartAvgScore: meta && !meta.error ? meta.avg_smart_score || 0 : 0,
...
function getSmartFollowersAnalysis(smartFollowers) {
  if (!smartFollowers || smartFollowers.error || !smartFollowers.smart_followers) {
    return {
      ...
      verifiedRatio: "0.00",
      quality: "Low",
      error: smartFollowers?.error || "No data"
    };
  }
...
function generateDetailedInsights(user, reputation, smartAnalysis) {
  ...
  if (smartAnalysis.error) {
    insights.push("📊 Умные подписчики недоступны - возможно, аккаунт не в базе TweetScout");
  } else if (smartAnalysis.quality === "High") {
    ...
    insights.push("👥 Нет умных подписчиков - возможно, аккаунт не анализировался TweetScout");
  }
...
async function sophireumFixedAnalysis() {
  const handle = 'sophireum';
  
  console.log('🔍 ИСПРАВЛЕННЫЙ АНАЛИЗ @sophireum');
  console.log('═'.repeat(80));
  console.log('📊 Правильная обработка ошибок API');
  console.log('═'.repeat(80));
  ...
  try {
    ...
    console.log(`✅ User API: ${user.error ? 'ОШИБКА' : 'УСПЕХ'}`);
    console.log(`✅ Smart API: ${smartFollowers.error ? 'ОШИБКА' : 'УСПЕХ'}`);
    console.log(`✅ Meta API: ${meta.error ? 'ОШИБКА' : 'УСПЕХ'}`);

    if (user.error) {
      console.log(`❌ Ошибка для @${handle}: ${user.error}`);
      return;
    }
    ...
    if (smartAnalysis.error) {
      console.log(`   ❌ Ошибка: ${smartAnalysis.error}`);
    }
    ...
    const smartBonus = (meta && !meta.error ? meta.avg_smart_score : 0) / 10;
    ...
    console.log(`\n🔍 ПОЧЕМУ УМНЫЕ ПОДПИСЧИКИ = 0:`);
    console.log('─'.repeat(50));
    console.log(`   API возвращает "Not Found" для умных подписчиков`);
    console.log(`   Это означает, что @${handle} не анализировался TweetScout`);
    console.log(`   или не имеет умных подписчиков в их базе данных`);
    console.log(`   Это нормально для некоторых аккаунтов`);
...
sophireumFixedAnalysis().catch(console.error); 