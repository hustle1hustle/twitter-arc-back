...
async function testV2Endpoint(path, description) {
...
async function testV2Endpoints() {
  ...
  console.log('🔍 ТЕСТИРОВАНИЕ V2 API ENDPOINTS');
  ...
  for (const endpoint of endpoints) {
    ...
    try {
      const result = await testV2Endpoint(endpoint.path, endpoint.desc);
...
testV2Endpoints().catch(console.error); 